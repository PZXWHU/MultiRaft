...
public class RaftNodeClusterChange1 {
    public static void main(String[] args) {
        ProxyConfig proxyConfig = new ProxyConfig().setDirectServerUrl("127.0.0.1:8002").setInvokeType(InvokeType.SYNC);
        RaftConsensusService raftConsensusService = proxyConfig.getProxy(RaftConsensusService.class);
        ClusterMembershipChangeRequest request = ClusterMembershipChangeRequest.builder().serverId(4).serverAddress("127.0.0.1:8004").build();
        ClusterMembershipChangeResponse response = raftConsensusService.addNode(request);
